...
// 1. Fetch the parcel details based on the ID passed in the URL
$qry = $conn->query("SELECT * FROM parcels where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}

// 2. Fetch branch addresses if 'to' or 'from' branch IDs are set
$branch = array();
			...
			<div class="col-md-6">
				<div class="callout callout-info">
					...
					<dl>
                        ...
                        <dd><?php echo ucwords( $branch[$from_branch_id] ?? 'Branch Not Found') ?></dd>
						<?php if($type == 2): ?>
							<dt>Nearest Branch to Recipient for Pickup:</dt>
                            <dd><?php echo ucwords($branch[$to_branch_id] ?? 'Branch Not Found') ?></dd>
						<?php endif; ?>
						<dt>Status:</dt>
						<dd>
							<?php 
